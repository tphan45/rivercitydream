...
gulp.task('build', gulp.series('partials', 'fonts', 'images', gulp.parallel('inject', 'other'), 'build'));
...
function watch(done) {
  ...
  gulp.watch(conf.path.src('scripts/**/*.html'), gulp.series('partials', reloadBrowserSync));
  gulp.watch([
    conf.path.src('**/*.scss'),
    conf.path.src('**/*.css')
  ], gulp.series('styles'));
